<p align="center">
  <h1 align="center">[更好的坐标导航]Better Coordinate Navigator</h1>
  <p align="center">
    轻量级 Minecraft 坐标标点与导航系统
    <br/>
    Entity-based markers + Screen direction guidance
    <br/><br/>
    <a href="#"><strong>📘 项目文档（开发中）</strong></a>
  </p>
</p>

<p align="center">
  <a href="https://Minecraft.net">
    <img src="https://img.shields.io/badge/Minecraft-Mod-62B47A?style=for-the-badge&logo=minecraft" />
  </a>
  <a href="https://www.java.com">
    <img src="https://img.shields.io/badge/Java-17+-F89820?style=for-the-badge&logo=java" />
  </a>
  <a href="https://qm.qq.com/q/e6kdQbTVja">
    <img src="https://img.shields.io/badge/QQ%E7%BE%A4-514280270-blue?style=for-the-badge" alt="QQ Group" />
  </a>
  <a href="https://github.com/FinNank1ng/Better-Coordinate-Navigator/blob/main/LICENSE">
    <img src="https://img.shields.io/badge/MIT License-yellow?style=for-the-badge" alt="License" />
  </a>
  <a href="https://github.com/FinNank1ng/Better-Coordinate-Navigator/stargazers">
    <img src="https://img.shields.io/github/stars/FinNank1ng/Better-Coordinate-Navigator?style=for-the-badge&color=F30525" />
  </a>
</p>

---

## 核心功能

- 实体化坐标标点（无碰撞体积，不影响游戏行为）
- 屏幕方向指引系统（实时指向目标坐标）
- 多标点管理与基础坐标解析
- 轻量级架构设计，便于扩展 UI / 数据同步
- 模块化设计，支持后续功能扩展

---

## 游戏内能力

- 在世界中创建“不可碰撞标记实体”
- 自动追踪目标坐标方向
- 屏幕实时显示导航指示
- 支持多标点切换与管理

<img width="2560" height="1600" alt="11f42dfc0530ace1b737d3beb9b4457f" src="https://github.com/user-attachments/assets/f1c91f8c-6fcc-4cc3-a715-59791347d226" />
---
